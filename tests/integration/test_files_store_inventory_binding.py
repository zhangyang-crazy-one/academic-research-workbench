...
def _open_writable_cache(store_path: Path) -> sqlite3.Connection:
    ...
    return sqlite3.connect(
        f"file:{quote(str(store_path))}?mode=rwc", uri=True
    )
...
def test_mutated_column_refused(
    ...
    try:
        if mutation == "relative_path":
            ...
            cache.execute(
                "UPDATE files SET source_digest = 'f' * 64 "
                "WHERE relative_path = 'notes/b.txt'"
            )
        elif mutation == "body":
            cache.execute(
                "UPDATE files SET body = 'TAMPERED' "
                "WHERE relative_path = 'notes/b.txt'"
            )
            cache.execute(
...
def test_every_operation_runs_inventory_check(
    ...
    if operation == "list_files":
        ...
        adapter.read_file(
            _read_request(root_id, file_id=file_id, relative_path=relative_path, digest=digest)
        )
