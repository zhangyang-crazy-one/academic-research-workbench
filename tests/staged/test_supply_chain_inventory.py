...
def test_validate_only_rejects_whole_header_alias_on_contract_sha256(
    ...
    validated = _validate_stage(stage_root)
    assert validated.returncode != 0
    # The verifier derives the expected value from the independently
    # regenerated header (the staged bytes are not authoritative), so the
    # whole-header alias is rejected because it does not match the
    # regenerated header's embedded value.
...
def test_validate_only_rejects_paired_header_and_identity_rebind(
    ...
    """RED: paired rewrite of header + identity digest + staged_payloads/inventory rejected."""

    stage_root = tmp_path / "paired-rebind-stage" / PLUGIN_NAME
    result = _stage(stage_root)
    assert result.returncode == 0, result.stderr
    ...
    contracts_path.write_bytes(fake_bytes)
