...
6. **Manuscript-artifact boundary** — final outputs must not contain user-facing
   explanation, planning commentary, audit prose, "I/you/user" wording, or text that
   was meant to explain design choices to the author rather than communicate to readers.
7. **Evidence-tier preservation** — preserve or surface labels that distinguish main
   full-validation results from screening, cross-model, oracle/diagnostic, semantic
   validation, and efficiency evidence; do not format diagnostics as main results.
...
Reference: `references/academic_pdf_format_reference.md`
...
## Venue Family Hard Packs (layout compliance)

Reference: `references/venue_family_hard_packs.md`

Venue layout rules differ (ACL ≠ NeurIPS ≠ IEEE ≠ Chinese GB). Do not apply one decorative “academic PDF” skin to every target.

### Preflight
1. Read PCR **Venue Family**. If the user asked for camera-ready / 顶会对齐 and the row is missing, ask once using the intake prompt in the hard-packs file.
2. If a family is declared, open `venue_family_hard_packs.md` and read **only that family's subsection** plus the shared black-heading norm.
3. Apply that family's hard table to template choice, paper size, fonts, caption scheme, reference engine, and link color.
4. In the Final Quality Checklist, emit a **Venue Family Compliance** block with PASS / FAIL / NOT-CHECKED per hard row.
5. If any hard row is FAIL **or** any length/limit row is NOT-CHECKED when the user asked for a camera-ready claim, do **not** label the package submission-ready or camera-ready for that family. Offer remediation or a family switch (e.g. `cn_tech_report` for design baselines).

Never infer family from the journal name alone. Numeric CFP limits stay scholar-declared or NOT-CHECKED — never FAIL solely on a remembered “typical” page count.

### Audit answer contract (when reviewing an existing PDF/TeX)
Follow `venue_family_hard_packs.md` § Audit / compliance answer contract:
- State the machine slug in backticks (`cn_tech_report`, `acl`, …).
- Emit a Compliance table with only `PASS` / `FAIL` / `NOT-CHECKED`.
- For Chinese design/architecture baselines that are not conference submissions, recommend `cn_tech_report` (not a free paraphrase as the identity).
- Refuse ACL/IEEE/NeurIPS camera-ready labels unless that family was declared and its hard rows pass.

## LaTeX Layout Export Gate

Compilation success is necessary but not sufficient for a scholarly PDF. For
two-column LaTeX output, source geometry, float scheduling, and the rendered
pages are three separate verification surfaces. Apply this gate whenever the
output includes `.tex` or PDF produced from LaTeX/Pandoc.

### Source preflight

1. Read the official class/style before changing paragraph layout. Record the
   effective `\parindent`, `\parskip`, and whether the class suppresses the
   first paragraph indent after a heading. Preserve those defaults globally.
   If deliberately parallel paragraphs in one summary block must align, use a
   narrowly scoped `\noindent`; do not set global `\parindent=0` to repair one
   paragraph.
2. Inventory source order for `figure`, `figure*`, `table`, `table*`, and every
   float barrier. Width and scheduling are independent decisions:
   - reserve `figure*`/`table*` for a dominant overview whose labels would be
     unreadable at `\columnwidth`;
   - keep secondary figures/tables single-column and fit the existing asset
     proportionally to `\columnwidth` (do not redraw, crop, or change its
     semantics merely to make it one-column);
   - never place `\FloatBarrier` while an earlier double-column float is still
     queued. A pending `table*`/`figure*` can otherwise force an interior blank
     column or large unexplained page gap.
3. Keep citation hyperlinks local to the cited claims. If PDF annotation or
   link-placement warnings occur, first split an overlong combined citation
   into claim-local citations; do not globally disable links as a layout fix.

### Build and rendered-page audit

1. Build from a clean auxiliary state when practical, then scan the complete
   log for fatal errors, unresolved citations/references, `Overfull` boxes, and
   PDF annotation warnings. Classify any accepted warning explicitly.
2. Render **every page** to PNG (or equivalent) and create a contact sheet.
   Inspect both the contact sheet and every suspicious page at readable zoom.
3. Reject the export when a non-terminal page has an unexplained empty column
   or roughly one quarter-page of avoidable whitespace, a detached caption,
   overlap/cropping, a float delayed beyond the section that interprets it, or
   inconsistent indentation among deliberately parallel paragraphs.
4. Reinspect downstream pages after every float-order/barrier change. Treat
   terminal bibliography whitespace separately from an interior scheduling
   defect.
5. Record page count, page size, font embedding, text extraction, compile-log
   findings, rendered-page coverage, and any accepted warnings in the delivery
   evidence. Do not claim camera-ready status from a successful compiler exit
   alone.
...
Reference: `references/venue_family_hard_packs.md` (family layout only — load one family)
...
- [ ] Venue family declared and matching the named journal's template family
- [ ] Word/page limit
- [ ] Abstract word limit
- [ ] Heading format (**black** headings unless the official template says otherwise)
...
- Prefer the official venue template for that family over a free-form `article` report skin
...
- [ ] Section/subsection headings are black (no decorative report colors) for PDF/TeX scholarly outputs
- [ ] Paragraph indentation follows the official class/style; deliberately parallel paragraph groups are visually consistent without a global override
- [ ] Font/spacing/margin specifications (if applicable)
- [ ] Page numbers (if applicable)
- [ ] Journal-specific requirements (if applicable)
- [ ] Two-column float width and declaration order audited: dominant overview floats may span both columns; secondary figures/tables remain proportionally fitted to one column
- [ ] Full-PDF rendered-page audit completed; no unexplained interior blank column, large avoidable gap, detached caption, overlap, or crop remains
- [ ] Venue Family Compliance block completed when a family is declared (or when camera-ready was requested): each hard-pack row PASS/FAIL/NOT-CHECKED; any FAIL blocks submission-ready labeling
...
| Venue Family | [acl / neurips / icml / ieee_trans / acm / gb_cn_journal / cn_tech_report / general / undeclared] |
| Word Count | [N] words |
...
- PDF: `pandoc paper.md -o paper.pdf --pdf-engine=xelatex -V mainfont="Times New Roman" -V CJKmainfont="Noto Serif CJK TC"`
```
...
Step 2: Content Pre-Processing
  ...
  2.6 Run a manuscript-artifact leak scan across body text, headings, figure labels,
      captions, table notes, graphical abstracts, and supplementary manuscript files.
      Remove or flag any assistant-user dialogue, planning commentary, "as you requested",
      "I recommend", "we discussed", "user-approved", "给你的解释", "我认为", "你需要",
      or equivalent non-manuscript wording.
  2.7 Confirm figures/tables that mix experimental stages preserve evidence-tier labels
      such as sample screening, full validation, cross-model check, oracle diagnostic,
      semantic-validation gate, or efficiency trade-off.
...
Step 5: Final Quality Check
  ...
  -> Execute the LaTeX Layout Export Gate for .tex/PDF outputs, including a
     full-page render contact sheet and source-order audit for two-column floats
...
□ Heading levels correct
□ Paragraph indentation matches the official class/style; parallel paragraph blocks are consistent
□ Font/line spacing/margins meet requirements
□ Page number position correct
□ Journal-specific requirements met (if applicable)

=== Rendered PDF Layout (LaTeX/PDF outputs) ===
□ Every page rendered and reviewed through a contact sheet; suspicious pages inspected at readable zoom
□ Main overview figures/tables use double-column width only when required for legibility
□ Secondary figures/tables preserve their content and are proportionally fitted to one column
□ `figure*`/`table*` declarations and float barriers cannot strand a pending double-column float
□ No unexplained interior blank column/large gap, detached caption, overlap, crop, or section-orphaned float
□ Page size, page count, font embedding, text extraction, compile-log warnings, and visual coverage recorded
