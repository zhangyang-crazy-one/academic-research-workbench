...
- **LaTeX/PDF layout-export gate (local ARW adapter).** The academic-paper formatter now treats compilation, float scheduling, and rendered-page quality as separate verification surfaces. It preserves official class-level paragraph indentation, permits only scoped `\noindent` exceptions for deliberately parallel blocks, separates one-/two-column asset sizing from float placement, prevents early `\FloatBarrier` use while starred floats are queued, and requires every PDF page plus a contact sheet to be inspected for interior blank columns, large avoidable gaps, detached captions, overlap, cropping, or section-orphaned floats. The PDF/LaTeX references, ACL hard pack, and workflow entry point carry the same contract. `check_layout_export_contract.py`, its mutation tests, and the pytest manifest entry prevent future adapter refreshes from silently dropping one surface.
