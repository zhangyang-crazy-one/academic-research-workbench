...
最後一個尤其是教訓：**我為了消假 veto（P2-e）動 negation 邏輯，反而引入新 false-positive**（內部回饋案例 `feedback_simplify_reviewer_careful_removing_safety`；該維護者筆記不隨發行快照分發）。補洞路線不只補不完，還會自傷。
...
5. 符合工具核心承諾與 LLM-defect-class posture（mitigate by narrowing the claim；內部回饋筆記 `feedback_llm_defect_class_problems_may_have_no_current_fix` 不隨發行快照分發）。
