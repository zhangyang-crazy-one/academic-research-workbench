...
| Science Workbench MVP | Root router loads a Codex-side run/audit/verification contract before routing to paper, pipeline, or experiment workflows | Full-runtime planner does not yet execute the Science Workbench pipeline directly | partial | `SKILL.md`, `codex/references/science_workbench_mvp.md` | skill quick validation; manual requirements trace | Execution scripts and connector adapters remain future work |
