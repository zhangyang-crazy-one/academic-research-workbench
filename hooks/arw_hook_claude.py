#!/usr/bin/env python3
"""Adapt Claude Code lifecycle hook wire events into bounded ARW observations.

Claude Code owns stdin/stdout for plugin hooks.  This executable emits only a
Claude-compatible observational output shape on stdout.  A redacted, immutable
receipt is written under the user data directory; the receipt cannot mutate a
run, admit evidence, retry work, or decide a gate.  Hook absence, distrust,
timeout, or failure consequently leaves every canonical control with the parent.
...
RECEIPT_SCHEMA = "arw.claude-hook-observation.v1"
SUPPORTED_EVENTS = frozenset({"SessionStart", "SubagentStop", "Stop"})
PARENT_CONTROLS = ("runtime", "mcp", "integrity", "gate", "provenance")
REQUIRED_COMMON = ("session_id", "cwd", "hook_event_name")
...
def _validate_claude_input(value: Mapping[str, Any]) -> str:
    """Loose Claude schema: require common fields; tolerate host-specific extras."""
    for field in REQUIRED_COMMON:
        if field not in value:
            raise HookWireError("missing-required-field")
    ...
    if "transcript_path" in value and not _bounded_string(
        value["transcript_path"], nullable=True, limit=4096
    ):
        raise HookWireError("invalid-transcript-path")
    if "permission_mode" in value and not _bounded_string(
        value["permission_mode"], nullable=True, limit=128
    ):
        raise HookWireError("invalid-permission-mode")
    if "model" in value and not _bounded_string(value["model"], nullable=True, limit=512):
        raise HookWireError("invalid-model")
    if event == "SessionStart" and "source" in value:
        if not _bounded_string(value["source"], nullable=True, limit=128):
...
def _resolve_plugin_root() -> Path:
    ...
    if root_value and os.path.isabs(root_value) and "\x00" not in root_value:
        try:
    ...
    # Claude does not always export CLAUDE_PLUGIN_ROOT into the child env; fall
    # back to this script's installed location (.../hooks/arw_hook_claude.py).
    try:
        script = Path(__file__).resolve(strict=True)
    except (OSError, RuntimeError):
        raise HookWireError("plugin-root-missing") from None
...
def _installed_definition_digest(root: Path) -> tuple[str, str]:
    components: list[bytes] = []
    for relative in ("hooks/hooks.json", "hooks/arw_hook_claude.py"):
...
def _data_root() -> Path:
    """Prefer PLUGIN_DATA; otherwise XDG/user data for Claude installs."""
    data_value = os.environ.get("PLUGIN_DATA")
    if data_value and os.path.isabs(data_value) and "\x00" not in data_value:
        return Path(data_value)

    xdg = os.environ.get("XDG_DATA_HOME")
    if xdg and os.path.isabs(xdg) and "\x00" not in xdg:
        return Path(xdg) / "academic-research-workbench"

    home = os.environ.get("HOME")
    if not home or not os.path.isabs(home) or "\x00" in home:
        raise HookWireError("plugin-data-missing")
    return Path(home) / ".local" / "share" / "academic-research-workbench"
...
def _redacted_receipt(
    ...
    *,
    ...
    receipt: dict[str, Any] = {
        ...
        "turn_id_sha256": _digest(value["turn_id"])
        if isinstance(value.get("turn_id"), str)
        else None,
        "subject_id_sha256": _digest(value["agent_id"])
        if isinstance(value.get("agent_id"), str)
        else None,
        "agent_type_sha256": _digest(value["agent_type"])
        if isinstance(value.get("agent_type"), str)
        else None,
        "model_sha256": _digest(model) if isinstance(model, str) else None,
        ...
        return {
            ...
            "hookSpecificOutput": {
                ...
                "additionalContext": (
                    ...
                    "ARS PreToolUse write-scope guard is optional hardening, not ledger "
                    "authority."
                ),
            },
        }
...
def main() -> int:
    ...
    try:
        ...
        event = _validate_claude_input(value)
        root = _resolve_plugin_root()
        definition_sha256, plugin_root_sha256 = _installed_definition_digest(root)
        ...
        sys.stderr.write(f"arw_hook_claude: {error.code}\n")
        return 1
