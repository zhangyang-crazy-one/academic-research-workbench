...
| 5 | Root hooks are directly supply-chain gated, nested hooks cannot substitute, and hooks remain observational with parent-only canonical authority. | ✓ VERIFIED | Exact root-hook SBOM binding remains the primary immutable gate. A never-executed default-deny AST layer independently rejects unauthorized imports, dynamic execution/reflection, direct writes, process/network calls, hidden binding forms and unsafe write receivers even when malicious handler bytes are rebound into SBOM. The current receipt primitives and all eight quality gates pass; root hook bytes remain unchanged. |
| 6 | Stage/schema/SBOM/provenance metadata is exact and self-consistent. | ✓ VERIFIED | Integration lock v2 binds the cycle-free build-identity metadata projection; live verification recomputes manifest coverage and closes the referenced host-canary evidence graph. Final clean-tree suite/ARS hashes are `95ba28cbdf7c733bf36b4e62fb26cb9e4e3e7e5da2433cd2b970f0f1121e1386` / `dc021dbca00df0459a6282e18276a0295c47b2432aaffc6462734ae8a503e903`; actual and declared SBOM SHA-256 are both `d6e48f5fdc05ab57bbb6539c6331a97d7822e1de7c41a9768df0a9092fb0e092`. Source freshness, quality gates and a fresh isolated stage all pass. |
