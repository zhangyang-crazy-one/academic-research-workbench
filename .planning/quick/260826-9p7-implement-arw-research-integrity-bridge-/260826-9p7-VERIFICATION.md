...
| --- | --- | --- | --- |
| 1 | Operators can distinguish the planned integration failures, and diagnostics cannot bypass the final exact verifier. | ✓ VERIFIED | Ten closed layers remain defined in `src/arw/integration_lock.py`; the three fresh-home receipts require distinct controlled-channel proposal digests as well as distinct nonces, and a verifier-injected final exact-verifier rejection still produced `BLOCKED/exact_lock_drift` after nine PASS layers. Plain and diagnostic route contracts remain fail-closed. |
| 2 | A strict ARS literature entry becomes a deterministic source -> span -> claim digest chain through installed schemas. | ✓ VERIFIED | `research_source_from_ars_entry` mirrors authoritative scalar and collection constraints without bridge-only caps: 257-item collections, >1 MiB private fields and 18 long-field variants round-trip through the exact bibliographic digest. Source, locator and span model inputs are canonical-byte strict-revalidated by each builder before any downstream artifact or digest is emitted; patterns, RFC3339, NUL, explicit-null and allOf/cross-field rules remain fail-closed. |
| 3 | Unicode/CJK filenames preserve display text and emit deterministic, collision-safe ASCII citekeys without Latin regression. | ✓ VERIFIED | The adapter retains separated Unicode grammar and legacy ASCII behavior while validating an unseparated non-ASCII pre-year family as one complete token. Mixed `张Chen2024.pdf` is preserved as family `张Chen`; `Chen🙂2024.pdf` and lowercase ASCII legacy misses are rejected. All 248 adapter tests passed. |
...
| 5 | Root hooks are directly supply-chain gated, nested hooks cannot substitute, and hooks remain observational with parent-only canonical authority. | ✓ VERIFIED | Exact root-hook SBOM binding pins bytes; an independent never-executed semantic AST fingerprint pins the single audited executable shape under Python 3.13/3.14. Comments and formatting normalize away, while every executable import/write/rebind/mutator or constant change fails even after a malicious handler digest is rebound into SBOM. The current handler and all eight quality gates pass; root hook bytes remain unchanged. |
| 6 | Stage/schema/SBOM/provenance metadata is exact and self-consistent. | ✓ VERIFIED | Integration lock v2 live-recomputes every staged digestPath/aggregate before binding the cycle-free projection. Each of the five staged evidence files must satisfy its full strict producer contract with `technical_qualification` derived from semantics: pre-vendor component rows are cross-bound field-by-field to the staged source manifest (regenerated receipt claims ARS 0.1.26 @ `8cc7f8f4` matching the manifest), legal rows are pinned per component and re-verified against live staged `LICENSES/` bytes, and `created_at` must parse as a real UTC calendar datetime. File-contract authority is independently regenerated from the staged checked schemas; native post-patch and upstream-test trees are pinned to verifier constants. Final suite/ARS hashes remain `1ef564d7131b087891f363032dccc7c92aa3096484b127d2c25947800e10e247` / `dc021dbca00df0459a6282e18276a0295c47b2432aaffc6462734ae8a503e903`; actual and declared SBOM SHA-256 are both `78d9c0d7b997b7259106e932a27a0ffb18e1a10776ae859ebc305ec060166c7f`. |
