...
| 3 | Unicode/CJK filenames preserve display text and emit deterministic, collision-safe ASCII citekeys without Latin regression. | ✓ VERIFIED | The adapter retains separated Unicode grammar and legacy ASCII behavior while validating an unseparated non-ASCII pre-year family as one complete token. Mixed `张Chen2024.pdf` is preserved as family `张Chen`; emoji, lowercase ASCII legacy misses, and complete-filename `Cc`/`Cf` controls (including bidi and zero-width controls) are rejected before title emission. All 28 folder-scan adapter tests passed. |
...
| 6 | Stage/schema/SBOM/provenance metadata is exact and self-consistent. | ✓ VERIFIED | Integration lock v2 live-recomputes every staged digestPath/aggregate. Pre-vendor and all three native five-file evidence bundles are canonical committed supply-chain artifacts; each surface pins its verdict+command raw SHA pair, so an upstream verdict cannot substitute for sanitizer execution. Every staged evidence JSON is parsed with duplicate-key and non-finite-number rejection before model validation, giving auditors one interpretation. Existing exact command/suite/test-tree/status, manifest/legal/live-byte and regenerated-contract checks remain. Final suite/ARS hashes are `1dba15e18d4b42e359a27ba44cb61365bec7b56b56ac02626f54a503141c2762` / `9c93b590653013ccbc288bad2c19105306f50bb04cf93ab0f8bb6ccd7afc91b0`; actual and declared SBOM SHA-256 are both `3beda7d22d0bb411e7f338fd36a2aacd6485832abf5dcb757ce0d81251d963da`. |
