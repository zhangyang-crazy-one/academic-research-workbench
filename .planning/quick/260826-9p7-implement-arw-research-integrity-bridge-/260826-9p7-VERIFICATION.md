...
| 2 | A strict ARS literature entry becomes a deterministic source -> span -> claim digest chain through installed schemas. | ✓ VERIFIED | `research_source_from_ars_entry` applies authoritative ARS nested and cross-field invariants without bridge-only collection caps: 257-author, tag and integrity-signal records round-trip through the bibliographic digest. Required/pattern/additionalProperties/allOf probes reject at both bundled and bridge boundaries; source/span/link builders recompute canonical upstream digests and whole-chain validation rejects replacement. |
