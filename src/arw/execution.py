...
# In-process memoization for the codex native binary digest.  The binary is
# 100-300 MB and every attempt re-qualifies it twice (TOCTOU recheck), so
# a full read + SHA-256 per call costs seconds per run.  The key covers the
# full stat tuple (device, inode, size, mtime, ctime) so any same-size
# rewrite still invalidates via ctime; stat failure is a cache miss (never a
# cached hit), and filesystems without nanosecond timestamps are handled by
# the AttributeError fallback that simply skips caching for that file.
_EXECUTABLE_DIGEST_CACHE: dict[tuple[int, int, int, int, int], tuple[str, str]] = {}
_EXECUTABLE_DIGEST_CACHE_MAX = 8
...
def _resolved_executable_digest(config: NativeHostConfig) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        if not resolved_candidate.is_file():
            return None, None
        stat_key: tuple[int, int, int, int, int] | None
        try:
            ...
            # Unreadable stat or a platform without nanosecond fields:
            # fail closed to an uncached full hash.
            stat_key = None
        if stat_key is not None and stat_key in _EXECUTABLE_DIGEST_CACHE:
            return _EXECUTABLE_DIGEST_CACHE[stat_key]
        digest = _hash_file(resolved_candidate)
        result = (str(resolved_candidate), digest)
        if stat_key is not None:
            if len(_EXECUTABLE_DIGEST_CACHE) >= _EXECUTABLE_DIGEST_CACHE_MAX:
                _EXECUTABLE_DIGEST_CACHE.clear()
            _EXECUTABLE_DIGEST_CACHE[stat_key] = result
        return result
    except OSError:
        return None, None
